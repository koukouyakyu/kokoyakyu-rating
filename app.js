(() => {
  ...
  const state = {
    ...
    ranking: [], prefs: [], proposals: [], ready: false, historyYears: '1', compareYears: '3', selectedSchoolKey: null, schoolSearchHits: [], recordSearchHits: [], compareSchoolKeys: [], rankCompareSearchHits: [], showDuplicatesOnly: false, duplicateGroups: []
  };
  ...
  function schoolMatchesArea(school,area){if(!area)return true;return Boolean(school?.areas?.has(area)||school?.pref===area);}
  ...
  function renderPrefFilter(){
    const old=els.prefFilter.value,source=ratingVisibleSchools(),special=['東京','東東京','西東京','北海道','南北海道','北北海道'];
  ...
  function renderRankCompareSearch(){
    ...
    const hits=source.filter(s=>!selected.has(s.key)&&(normalizeSearchText(s.name).includes(q)||normalizeSearchText(s.pref).includes(q)||[...(s.areas||[])].some(a=>normalizeSearchText(a).includes(q)))).slice(0,20);
