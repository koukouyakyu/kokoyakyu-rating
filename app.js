(() => {
  ...
  async function loadRatingCache(){
    ...
    try{
      ...
      state.cacheMeta=metaResp.data;state.cacheActualMeta=actualMeta;state.schoolMap=map;state.schools=schools;state.ranking=publicSchools;state.prefs=buildPrefStats(schools);state.matches=[];state.fullMatchesLoaded=false;state.cacheMode=true;state.ready=true;markMatchDataChanged();setDataStatus(cacheStatusLabel());renderAll(true);return true;
    }catch(error){
  ...
  async function ensureFullMatches(reason='詳細データ', forceRefresh=false){
    ...
    state.fullMatchesPromise=(async()=>{
      ...
      try{
        ...
        const r=buildRatings(matches);state.schoolMap=r.map;state.schools=r.schools;state.ranking=r.publicSchools;state.prefs=buildPrefStats(r.schools);state.fullMatchesLoaded=true;state.cacheMode=false;state.ready=true;markMatchDataChanged();setDataStatus(`${matches.length.toLocaleString('ja-JP')}試合`);
  ...
  async function persistRatingCache({silent=false}={}){
    ...
    try{
      ...
      state.ratingCacheAvailable=true;state.cacheMeta=meta;state.cacheActualMeta={match_count:meta.match_count,first_match_date:meta.first_match_date,latest_match_date:meta.latest_match_date};
  ...
  function renderPrefShareTrend(){
    ...
    // 高速表示中は最新年だけをキャッシュから即表示。過去推移はユーザーが必要な時だけ全試合を読む。
    if(!state.fullMatchesLoaded){
      ...
      const active=String(state.prefShareSortYear||'')===String(year),sortDir=state.prefShareSortDir==='asc'?'asc':'desc',indicator=active?(sortDir==='desc'?'▼':'▲'):'↕';
      ...
      if(active)prefs.sort((a,b)=>{const av=counts.get(a)||0,bv=counts.get(b)||0,diff=sortDir==='asc'?av-bv:bv-av;return diff||comparePrefectureOrder(a,b);});
      ...
      if(els.prefShareStatus){
        const sortNote=active?` ${year}年を${sortDir==='desc'?'降順':'昇順'}で表示中。`:'';
        els.prefShareStatus.innerHTML=`最新年は高速表示キャッシュから表示しています。${sortNote} 過去推移は必要なときだけ全試合を読み込みます。 <button id="prefShareLoadHistory" class="btn secondary small" type="button">過去推移を読み込む</button>`;
        els.prefShareStatus.querySelector('#prefShareLoadHistory')?.addEventListener('click',async()=>{const b=els.prefShareStatus.querySelector('#prefShareLoadHistory');if(b)b.disabled=true;const ok=await ensureFullMatches('都道府県割合推移用の全試合');if(ok)renderPrefShareTrend();});
      }
      return;
    }
    ...
    if(sortYear){
      ...
      if(col)prefs.sort((a,b)=>{const av=col.counts.get(a)||0,bv=col.counts.get(b)||0,diff=sortDir==='asc'?av-bv:bv-av;return diff||comparePrefectureOrder(a,b);});
    }
  ...
  async function restoreSession(){state.authMode=detectAuthModeFromUrl();const ae=getAuthErrorFromUrl();if(ae)showSetupNotice(`<strong>認証リンクを処理できませんでした。</strong> ${escapeHtml(ae)}`);state.client.auth.onAuthStateChange((event,session)=>{state.session=session;if(event==='PASSWORD_RECOVERY')state.authMode='recovery';else if(event==='SIGNED_IN'&&detectAuthModeFromUrl()==='invite')state.authMode='invite';renderAuth();if(state.ready){renderAll();if(session?.user&&!state.fullMatchesLoaded)ensureFullMatches('管理画面用の全試合').then(ok=>{if(ok)renderAuth();});}});const {data,error}=await state.client.auth.getSession();if(!error){state.session=data.session;renderAuth();if(state.ready)renderAll();}}
  ...
  function deferAdminFullMatches(){
    if(!isAdmin()||state.fullMatchesLoaded)return;
    const adminSection=document.getElementById('admin');
    const load=async()=>{if(state.fullMatchesLoaded||state.fullMatchesPromise)return;await ensureFullMatches('管理画面用の全試合');};
    if(location.hash==='#admin'){load();return;}
    if('IntersectionObserver' in window&&adminSection){
      const observer=new IntersectionObserver(entries=>{if(entries.some(x=>x.isIntersecting)){observer.disconnect();load();}},{rootMargin:'350px 0px'});
      observer.observe(adminSection);
    }else if(adminSection){
      adminSection.addEventListener('mouseenter',load,{once:true});
      adminSection.addEventListener('focusin',load,{once:true});
    }
  }
  async function init(){bindEvents();updateProposalMode();renderStaticConfig();renderSimulator();resetMatchForm();if(!isConfigReady()){setDataStatus('要設定');showSetupNotice('<strong>Supabaseの接続情報が未設定です。</strong> config.js を設定してください。');els.loginPanel.classList.add('hidden');els.adminUnavailable.textContent='config.jsを設定すると利用できます。';els.adminUnavailable.classList.remove('hidden');return;}state.client=window.supabase.createClient(cfg.supabaseUrl,cfg.supabasePublishableKey);els.adminUnavailable.classList.add('hidden');await Promise.all([loadSiteSettings(),loadTournamentKSettings()]);await loadSchoolAliases();await restoreSession();const cached=await loadRatingCache();if(isAdmin()){if(cached){renderAuth();deferAdminFullMatches();}else{const ok=await ensureFullMatches('管理画面用の全試合');if(ok)renderAuth();}}else if(!cached){await ensureFullMatches('初回表示用の全試合');}}
