(() => {
  ...
  const state = {
    ...
    ranking: [], prefs: [], proposals: [], ready: false, historyYears: '1', compareYears: '3', visibleGames: 20, selectedSchoolKey: null, schoolSearchHits: [], recordSearchHits: []
  };
  ...
  const els = {
    ...
    rankSchool1:$('rankSchool1'), rankSchool2:$('rankSchool2'), rankSchool3:$('rankSchool3'), rankSchool4:$('rankSchool4'), rankCompareRange:$('rankCompareRange'), rankCompareStatus:$('rankCompareStatus'), rankCompareChart:$('rankCompareChart'), rankCompareLegend:$('rankCompareLegend'),
  ...
  function renderAll(){hideSetupNotice();renderSummary();renderPrefFilter();renderRanking();renderPrefCards();renderSchoolSelect();renderSchoolProfile();renderRankCompareSelects();renderRankCompareChart();renderAdminMatches();}
  ...
  function renderRankCompareSelects(){
    if(!els.rankSchool1)return;const source=ratingVisibleSchools(),selects=[els.rankSchool1,els.rankSchool2,els.rankSchool3,els.rankSchool4],old=selects.map(x=>x.value),defaults=source.slice(0,4).map(s=>s.key);
    const opts='<option value="">— 選択しない —</option>'+source.map(s=>`<option value="${escapeHtml(s.key)}">${escapeHtml(s.name)}（${escapeHtml(s.pref)}）</option>`).join('');
    selects.forEach((el,i)=>{el.innerHTML=opts;const candidate=source.some(s=>s.key===old[i])?old[i]:(old[i]?'':defaults[i]||'');el.value=candidate;});
  }
  function renderRankCompareChart(){
    if(!els.rankCompareChart)return;const selects=[els.rankSchool1,els.rankSchool2,els.rankSchool3,els.rankSchool4],chosen=selects.map(x=>resolveSchool(x?.value)).filter(Boolean).filter((s,i,a)=>a.findIndex(x=>x.key===s.key)===i).slice(0,4);
    if(!chosen.length){els.rankCompareChart.innerHTML='<text x="450" y="170" text-anchor="middle" class="chart-private">比較する学校を選択してください</text>';els.rankCompareLegend.innerHTML='';return;}
