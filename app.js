(() => {
  ...
  function matchK(match) {
    ...
    const tournamentSpecific =
      Number(
        ratingCfg.tournamentK?.[
          match.tournament
        ]
      );

    if (
      Number.isFinite(tournamentSpecific) &&
      tournamentSpecific > 0
    ) {
  ...
  function normalizeMatch(match) {
    return {
      ...
      team_a:
        String(match.team_a ?? "").trim(),

      pref_a:
        String(match.pref_a ?? "").trim(),

      team_b:
        String(match.team_b ?? "").trim(),

      pref_b:
        String(match.pref_b ?? "").trim(),

      tournament:
        String(match.tournament ?? "").trim(),

      stage:
        String(match.stage ?? "").trim(),
  ...
  function showSetupNotice(html) {
    ...
    els.setupNotice.classList.remove(
      "hidden"
    );
  }

  function hideSetupNotice() {
    els.setupNotice.classList.add(
      "hidden"
    );
  ...
  function renderSummary() {
    ...
    els.schoolCount.textContent =
      state.schools.length
        .toLocaleString(
          "ja-JP"
        );

    els.prefCount.textContent =
      state.prefs.length
        .toLocaleString(
          "ja-JP"
        );
    ...
    els.latestMatchDate.textContent =
      latest || "—";
  }

  function renderPrefFilter() {
    const selected =
      els.prefFilter.value;
    ...
    els.prefFilter.innerHTML =
      ...
      prefs
        .map(
          (pref) =>
            `<option value="${escapeHtml(pref)}">${escapeHtml(pref)}</option>`
        )
        .join("");
            ...
            (school) => {
              const formHtml =
                school.form
                  ...
                  .map(
                    (r) =>
                      `<span class="match-result-${r}">${escapeHtml(r)}</span>`
                  )
                  .join(" ") ||
                "—";
      ...
      .forEach(
        (row) => {
          row.addEventListener(
            "click",
            () => {
              els.schoolSelect.value =
                row.dataset.schoolKey;
          ...
          (
            ...
            <article class="pref-card">
              ...
              <div class="pref-metrics">
                <div>
                  <span>平均</span>
                  <strong>${formatRating(item.average)}</strong>
                </div>

                <div>
                  <span>中央値</span>
                  <strong>${formatRating(item.median)}</strong>
                </div>

                <div>
                  <span>Top5平均</span>
                  <strong>${formatRating(item.top5)}</strong>
                </div>

                <div>
                  <span>Top25%平均</span>
                  <strong>${formatRating(item.top25)}</strong>
                </div>
              </div>
            </article>
          `
  ...
  function renderSchoolProfile() {
    ...
    if (!school) {
      ...
      els.recentMatches.innerHTML =
        `<div class="empty">試合データがありません。</div>`;
    ...
    els.schoolForm.innerHTML =
      school.form
        ...
        .map(
          (r) =>
            `<span class="match-result-${r}">${r}</span>`
        )
        .join(" ") ||
      "—";
          ...
          (game) => {
            ...
            return `
              <div class="match-row">
                ...
                <div class="match-meta">
                  ${formatRating(game.before)}
                  →
                  ${formatRating(game.after)}
  ...
  function renderAuth() {
    ...
    els.loginPanel.classList.toggle(
      "hidden",
      signedIn ||
        needsPasswordSetup
    );

    els.resetRequestPanel.classList.add(
      "hidden"
    );

    els.passwordSetupPanel.classList.toggle(
      "hidden",
      !needsPasswordSetup
    );

    els.adminPanel.classList.toggle(
      "hidden",
      !signedIn ||
        needsPasswordSetup
    );
    ...
    if (needsPasswordSetup) {
      ...
      els.passwordSetupTitle.textContent =
        isInvite
          ? "初回パスワードを設定"
          : "新しいパスワードを設定";

      els.passwordSetupDescription.textContent =
        isInvite
  ...
  function showResetRequest() {
    ...
    els.loginPanel.classList.add(
      "hidden"
    );

    els.resetRequestPanel.classList.remove(
      "hidden"
    );
  ...
  function backToLogin() {
    ...
    els.loginPanel.classList.remove(
      "hidden"
    );
  ...
  function resetMatchForm() {
    ...
    els.matchFormTitle.textContent =
      "試合を登録";

    els.saveMatchButton.textContent =
      "試合を登録";

    els.cancelEditButton.classList.add(
      "hidden"
    );
  ...
  function startEdit(id) {
    ...
    els.cancelEditButton.classList.remove(
      "hidden"
    );
          ...
          (m) => `
            <tr>
              ...
              <td>
                ...
                ${
                  m.stage
                    ? `<br><span class="muted">${escapeHtml(m.stage)}</span>`
                    : ""
                }
              </td>
              ...
              <td>
                <div class="action-buttons">
                  <button
                    class="btn secondary small js-edit"
                    type="button"
  ...
  function bindEvents() {
    ...
    els.prefFilter.addEventListener(
      "change",
      renderRanking
    );

    els.schoolSelect.addEventListener(
      "change",
      renderSchoolProfile
    );
    ...
    els.showResetButton.addEventListener(
      "click",
      showResetRequest
    );

    els.resetRequestForm.addEventListener(
      "submit",
      handleResetRequest
    );

    els.backToLoginButton.addEventListener(
      "click",
      backToLogin
    );

    els.passwordSetupForm.addEventListener(
      "submit",
      handlePasswordSetup
    );
    ...
    els.matchForm.addEventListener(
      "submit",
      handleMatchSubmit
    );

    els.cancelEditButton.addEventListener(
      "click",
      () => {
        resetMatchForm();
    ...
    els.reloadButton.addEventListener(
      "click",
      loadMatches
    );
  }
  ...
  function renderStaticConfig() {
    els.heroInitial.textContent =
      String(
        ratingCfg.initial
      );

    els.heroDivisor.textContent =
      String(
        ratingCfg.divisor
      );

    els.heroK.textContent =
      String(
        ratingCfg.defaultK
      );

    els.heroFormula.textContent =
      `R' = R + ${ratingCfg.defaultK} × (W − We)`;
  ...
  async function init() {
    ...
    if (!isConfigReady()) {
      ...
      els.passwordSetupPanel.classList.add(
        "hidden"
      );
    ...
    if (
      ...
      showSetupNotice(
        "Supabase JavaScriptライブラリを読み込めませんでした。ネットワーク接続を確認してください。"
      );
    ...
    els.adminUnavailable.classList.add(
      "hidden"
    );
