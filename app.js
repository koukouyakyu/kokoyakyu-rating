(() => {
  ...
  const DEFAULT_SITE_SETTINGS = Object.freeze({
    id: 1,
    public_k_values: true, public_ranking: true, public_pref_share: true, public_prefectures: true, public_school_details: true,
  ...
  const state = {
    ...
    selectedRankingPrefs: new Set(), prefShareTopN: 100, prefShareSortYear: null, prefShareSortDir: 'desc', editingSchoolAliasId: null, tournamentAliases: [], tournamentAliasesAvailable: true, tournamentAliasMap: new Map(), editingTournamentAliasId: null,
    fullMatchesLoaded: false, fullMatchesPromise: null, ratingCacheAvailable: true, cacheMode: false, cacheMeta: null, cacheActualMeta: null, schoolSelectPopulated: false, lazyRenderedSections: new Set(), lazySectionObserver: null
  };
  const $ = (id) => document.getElementById(id);
  const els = {
    dataStatus:$('dataStatus'), setupNotice:$('setupNotice'), searchInput:$('searchInput'), rankingPrefChooser:$('rankingPrefChooser'), rankingPrefSummary:$('rankingPrefSummary'), rankingPrefOptions:$('rankingPrefOptions'), rankingPrefSelectAll:$('rankingPrefSelectAll'), rankingPrefClear:$('rankingPrefClear'), ratingMinFilter:$('ratingMinFilter'), ratingMaxFilter:$('ratingMaxFilter'), rankingDate:$('rankingDate'), rankingDateLatest:$('rankingDateLatest'), rankingPageSize:$('rankingPageSize'), rankingPrev:$('rankingPrev'), rankingNext:$('rankingNext'), rankingPageNumbers:$('rankingPageNumbers'), rankingPageStatus:$('rankingPageStatus'),
    rankingBody:$('rankingBody'), rankingFootnote:$('rankingFootnote'), matchCount:$('matchCount'), schoolCount:$('schoolCount'),
    prefCount:$('prefCount'), latestMatchDate:$('latestMatchDate'), prefShareTitle:$('prefShareTitle'), prefShareTopN:$('prefShareTopN'), prefShareStatus:$('prefShareStatus'), prefShareHead:$('prefShareHead'), prefShareBody:$('prefShareBody'), prefCards:$('prefCards'), prefSort:$('prefSort'), prefShowMoreButton:$('prefShowMoreButton'),
    ...
    adminPanel:$('adminPanel'), adminEmail:$('adminEmail'), logoutButton:$('logoutButton'), ratingCacheRebuildButton:$('ratingCacheRebuildButton'), ratingCacheAdminStatus:$('ratingCacheAdminStatus'), matchForm:$('matchForm'), matchFormTitle:$('matchFormTitle'),
  ...
  try{
    const savedPrefShareTopN=Math.trunc(Number(localStorage.getItem('kokoyakyu_pref_share_top_n')));
    if(Number.isFinite(savedPrefShareTopN)&&savedPrefShareTopN>=1&&savedPrefShareTopN<=1000){
      state.prefShareTopN=savedPrefShareTopN;
      if(els.prefShareTopN)els.prefShareTopN.value=String(savedPrefShareTopN);
    }
  }catch(_){}
  let globalRatingReadyResolved=false;
  let resolveGlobalRatingReady=null;
  if(!window.KOKOYAKYU_RATING_CACHE_READY){
    window.KOKOYAKYU_RATING_CACHE_READY=new Promise(resolve=>{resolveGlobalRatingReady=resolve;});
  }
  function publishGlobalRatingSnapshot(value){
    const rows=Array.isArray(value)?value:state.schools;
    if(Array.isArray(rows)&&rows.length){
      window.KOKOYAKYU_RATING_SNAPSHOT=rows.map(s=>({
        school_key:s.key,school_name:s.name,prefecture:canonicalPref(s.pref),rating:Number(s.rating),is_joint:Boolean(s.isJoint),active_last_year:Boolean(s.activeLastYear??hasOfficialGameInPastYear(s)),all_rank:Number(s.allRank)||null,public_rank:Number(s.publicRank)||null,pref_rank:Number(s.prefRank)||null
      }));
    }
    if(!globalRatingReadyResolved&&resolveGlobalRatingReady){globalRatingReadyResolved=true;resolveGlobalRatingReady(window.KOKOYAKYU_RATING_SNAPSHOT||null);}
    window.dispatchEvent(new CustomEvent('kokoyakyu:ratings-ready',{detail:window.KOKOYAKYU_RATING_SNAPSHOT||null}));
  }
  ...
  const PREFECTURE_ORDER=Object.freeze(['北海道','青森','岩手','宮城','秋田','山形','福島','茨城','栃木','群馬','埼玉','千葉','東京','神奈川','新潟','富山','石川','福井','山梨','長野','岐阜','静岡','愛知','三重','滋賀','京都','大阪','兵庫','奈良','和歌山','鳥取','島根','岡山','広島','山口','徳島','香川','愛媛','高知','福岡','佐賀','長崎','熊本','大分','宮崎','鹿児島','沖縄']);
  const PREFECTURE_ORDER_INDEX=new Map(PREFECTURE_ORDER.map((pref,index)=>[pref,index]));
  function comparePrefectureOrder(a,b){
    const pa=canonicalPref(a),pb=canonicalPref(b),ia=PREFECTURE_ORDER_INDEX.get(pa),ib=PREFECTURE_ORDER_INDEX.get(pb);
    if(ia!==undefined&&ib!==undefined)return ia-ib;
    if(ia!==undefined)return -1;if(ib!==undefined)return 1;
  ...
  function teamGenerationYear(match){
  ...
  function buildRatings(raw){
    ...
    for(let matchIndex=0;matchIndex<matches.length;matchIndex++){
      ...
      const gameNumberA=a.games.length+1,gameNumberB=b.games.length+1;
      if(preTransitionsA>0)a.history.push({date:m.date,rating:preRegressionToA,delta:0,opponent:null,matchId:null,eventType:'regression',newTeamTransitions:preTransitionsA,regressionFrom:preRegressionFromA,regressionTo:preRegressionToA,regressionTiming:'before_match',gameNumber:a.games.length,tournamentType:type});
      if(preTransitionsB>0)b.history.push({date:m.date,rating:preRegressionToB,delta:0,opponent:null,matchId:null,eventType:'regression',newTeamTransitions:preTransitionsB,regressionFrom:preRegressionFromB,regressionTo:preRegressionToB,regressionTiming:'before_match',gameNumber:b.games.length,tournamentType:type});
      a.history.push({date:m.date,rating:matchAfterA,delta:dA,opponent:b.name,matchId:m.id,eventType:'match',newTeamTransitions:0,matchAfter:matchAfterA,isAutumnFirstGame:autumnFirstA,tournamentType:type,gameNumber:gameNumberA});
      b.history.push({date:m.date,rating:matchAfterB,delta:dB,opponent:a.name,matchId:m.id,eventType:'match',newTeamTransitions:0,matchAfter:matchAfterB,isAutumnFirstGame:autumnFirstB,tournamentType:type,gameNumber:gameNumberB});
      if(postTransitionsA>0)a.history.push({date:m.date,rating:postRegressionToA,delta:0,opponent:null,matchId:null,eventType:'regression',newTeamTransitions:postTransitionsA,regressionFrom:postRegressionFromA,regressionTo:postRegressionToA,regressionTiming:'after_summer_final',gameNumber:gameNumberA,tournamentType:type});
      if(postTransitionsB>0)b.history.push({date:m.date,rating:postRegressionToB,delta:0,opponent:null,matchId:null,eventType:'regression',newTeamTransitions:postTransitionsB,regressionFrom:postRegressionFromB,regressionTo:postRegressionToB,regressionTiming:'after_summer_final',gameNumber:gameNumberB,tournamentType:type});
      a.games.push({match:m,result:cA,opponent:b.name,opponentPref:b.pref,opponentKey:b.key,scored:m.score_a,allowed:m.score_b,before:beforeA,matchAfter:matchAfterA,after:matchAfterA,delta:dA,newTeamTransitions:transitionCountA,preNewTeamTransitions:preTransitionsA,postNewTeamTransitions:postTransitionsA,regressionFrom:transitionCountA?(preTransitionsA?preRegressionFromA:postRegressionFromA):null,regressionTo:transitionCountA?(postTransitionsA?postRegressionToA:preRegressionToA):null,isAutumnFirstGame:autumnFirstA});
      b.games.push({match:m,result:cB,opponent:a.name,opponentPref:a.pref,opponentKey:a.key,scored:m.score_b,allowed:m.score_a,before:beforeB,matchAfter:matchAfterB,after:matchAfterB,delta:dB,newTeamTransitions:transitionCountB,preNewTeamTransitions:preTransitionsB,postNewTeamTransitions:postTransitionsB,regressionFrom:transitionCountB?(preTransitionsB?preRegressionFromB:postRegressionFromB):null,regressionTo:transitionCountB?(postTransitionsB?postRegressionToB:preRegressionToB):null,isAutumnFirstGame:autumnFirstB});
    }
  ...
  async function fetchRatingCacheMeta(){
    if(!state.client)return {data:null,error:new Error('Supabase client unavailable')};
    const columns='id,match_count,school_count,active_school_count,first_match_date,latest_match_date,built_at,cache_version,is_dirty';
    let res=await state.client.from('rating_cache_meta').select(columns).eq('id',1).maybeSingle();
    if(res.error&&/is_dirty/i.test(res.error.message||'')){
      res=await state.client.from('rating_cache_meta').select('id,match_count,school_count,active_school_count,first_match_date,latest_match_date,built_at,cache_version').eq('id',1).maybeSingle();
      if(!res.error&&res.data)res.data={...res.data,is_dirty:false};
    }
    return res;
  }
  async function fetchRatingCacheRows(){
    const columns='school_key,school_name,prefecture,rating,last_delta,wins,losses,draws,form,is_joint,all_rank,public_rank,pref_rank,first_game_date,last_game_date,active_last_year';
    const all=[];
    for(let from=0;;from+=PAGE_SIZE){
      const r=await state.client.from('rating_current').select(columns).order('all_rank',{ascending:true}).range(from,from+PAGE_SIZE-1);
  ...
  function schoolFromRatingCache(row){
    const pref=canonicalPref(row.prefecture),name=String(row.school_name||'').trim(),key=String(row.school_key||schoolKey(name,pref));
    const rating=Number(row.rating),lastDelta=Number(row.last_delta||0),wins=Number(row.wins||0),losses=Number(row.losses||0),draws=Number(row.draws||0);
    return {key,name,pref,rating:Number.isFinite(rating)?rating:ratingCfg.initial,lastDelta:Number.isFinite(lastDelta)?lastDelta:0,wins,losses,draws,form:String(row.form||''),isJoint:Boolean(row.is_joint),allRank:Number(row.all_rank)||null,publicRank:Number(row.public_rank)||null,prefRank:Number(row.pref_rank)||null,rank:Number(row.public_rank)||null,firstGameDate:row.first_game_date||null,lastGameDate:row.last_game_date||null,activeLastYear:Boolean(row.active_last_year),history:[],games:[],areas:new Set([pref]),cacheOnly:true,lastTransitionYear:null,autumnSeasonYears:new Set()};
  }
  function cacheIsStale(){return Boolean(state.cacheMeta?.is_dirty);}
  function cacheStatusLabel(){
    const count=Number(state.cacheMeta?.match_count||0).toLocaleString('ja-JP');
    return cacheIsStale()?`${count}試合 · 高速表示（更新待ち）`:`${count}試合 · 高速表示`;
  }
  async function loadRatingCache(){
    if(!state.client)return false;
    try{
      const [metaResp,rows]=await Promise.all([
        fetchRatingCacheMeta(),
        fetchRatingCacheRows()
      ]);
      if(metaResp.error)throw metaResp.error;
      if(!metaResp.data||!rows.length)return false;
      const schools=rows.map(schoolFromRatingCache).sort((a,b)=>(a.allRank||999999)-(b.allRank||999999)||a.name.localeCompare(b.name,'ja'));
      const map=new Map(schools.map(x=>[x.key,x]));
      const publicSchools=schools.filter(s=>s.publicRank!==null&&s.rating>=PUBLIC_RATING_MIN).sort((a,b)=>(a.publicRank||999999)-(b.publicRank||999999));
      state.cacheMeta=metaResp.data;state.cacheActualMeta=null;state.schoolMap=map;state.schools=schools;state.ranking=publicSchools;state.prefs=buildPrefStats(schools);state.matches=[];state.fullMatchesLoaded=false;state.cacheMode=true;state.ready=true;markMatchDataChanged();setDataStatus(cacheStatusLabel());publishGlobalRatingSnapshot(schools);renderAll(true);return true;
    }catch(error){
      state.ratingCacheAvailable=false;
      if(!globalRatingReadyResolved&&resolveGlobalRatingReady){globalRatingReadyResolved=true;resolveGlobalRatingReady(null);}
      console.warn('高速表示キャッシュを利用できません。従来方式へ切り替えます。',error);
      return false;
    }
  }
  async function ensureFullMatches(reason='詳細データ', forceRefresh=false){
    ...
    state.fullMatchesPromise=(async()=>{
      setDataStatus(`${reason}を読み込み中…`);
      try{
        const matches=await fetchAllMatches();
        state.matches=matches;state.missingTournamentTypeCount=matches.filter(m=>!m.tournament_type).length;
        const r=buildRatings(matches);state.schoolMap=r.map;state.schools=r.schools;state.ranking=r.publicSchools;state.prefs=buildPrefStats(r.schools);state.fullMatchesLoaded=true;state.cacheMode=false;state.ready=true;markMatchDataChanged();setDataStatus(`${matches.length.toLocaleString('ja-JP')}試合`);publishGlobalRatingSnapshot(r.schools);
        if(state.missingTournamentTypeCount)console.warn(`大会タグ未設定: ${state.missingTournamentTypeCount}試合。upgrade-tournament-tags.sql を実行してください。`);
        renderAll(true);return true;
      }catch(e){console.error(e);setDataStatus('読込エラー');showSetupNotice(`Supabaseから試合データを取得できませんでした。<br><code>${escapeHtml(e.message)}</code>`);return false;}
      finally{state.fullMatchesPromise=null;}
    })();
    return state.fullMatchesPromise;
  }
  function currentRatingCacheRows(){
    return state.schools.map(s=>({
      school_key:s.key,school_name:s.name,prefecture:canonicalPref(s.pref),rating:Number(s.rating),last_delta:Number(s.lastDelta||0),wins:Number(s.wins||0),losses:Number(s.losses||0),draws:Number(s.draws||0),form:String(s.form||''),is_joint:Boolean(s.isJoint),all_rank:Number(s.allRank)||null,public_rank:Number(s.publicRank)||null,pref_rank:Number(s.prefRank)||null,first_game_date:s.firstGameDate||s.history?.[0]?.date||null,last_game_date:s.games?.at(-1)?.match?.date||s.history?.at(-1)?.date||null,active_last_year:hasOfficialGameInPastYear(s),updated_at:new Date().toISOString()
    }));
  }
  async function persistRatingCache({silent=false}={}){
    if(!state.client||!state.session?.user||!state.fullMatchesLoaded)return false;
    const button=els.ratingCacheRebuildButton,status=els.ratingCacheAdminStatus;
    if(button)button.disabled=true;if(status&&!silent)status.textContent='高速表示キャッシュを保存中…';
    try{
      const rows=currentRatingCacheRows();
      const dates=state.matches.map(m=>m.date).filter(Boolean).sort();
      const meta={id:1,match_count:state.matches.length,school_count:state.schools.filter(s=>!s.isJoint).length,active_school_count:state.schools.filter(s=>!s.isJoint&&hasOfficialGameInPastYear(s)).length,first_match_date:dates[0]||null,latest_match_date:dates.at(-1)||null,built_at:new Date().toISOString(),cache_version:2,is_dirty:false};
      const rpc=await state.client.rpc('replace_rating_cache',{p_rows:rows,p_meta:meta});
      if(rpc.error){
        console.warn('replace_rating_cache RPC unavailable; batch fallback:',rpc.error.message);
        const del=await state.client.from('rating_current').delete().neq('school_key','');if(del.error)throw del.error;
        for(let i=0;i<rows.length;i+=500){const r=await state.client.from('rating_current').upsert(rows.slice(i,i+500),{onConflict:'school_key'});if(r.error)throw r.error;}
        let mr=await state.client.from('rating_cache_meta').upsert(meta,{onConflict:'id'});if(mr.error&&/is_dirty/i.test(mr.error.message||'')){const legacy={...meta};delete legacy.is_dirty;mr=await state.client.from('rating_cache_meta').upsert(legacy,{onConflict:'id'});}if(mr.error)throw mr.error;
      }
      state.ratingCacheAvailable=true;state.cacheMeta=meta;state.cacheActualMeta=null;publishGlobalRatingSnapshot(state.schools);
      if(status)status.textContent=`更新完了：${meta.match_count.toLocaleString('ja-JP')}試合・${rows.length.toLocaleString('ja-JP')}チーム（${new Date(meta.built_at).toLocaleString('ja-JP')}）`;
      return true;
    }catch(e){console.error(e);if(status)status.textContent=`キャッシュ更新に失敗しました: ${e.message||e}`;return false;}
  ...
  function markMatchDataChanged(){state.rankSnapshotCache.clear();state.historicalRankingCache.clear();state.duplicateCacheDirty=true;state.matchSortCache=null;state.schoolSelectPopulated=false;}
  function rebuildFromLocalMatches(deferSecondary=true){const r=buildRatings(state.matches);state.schoolMap=r.map;state.schools=r.schools;state.ranking=r.publicSchools;state.prefs=buildPrefStats(r.schools);state.missingTournamentTypeCount=state.matches.filter(m=>!m.tournament_type).length;state.fullMatchesLoaded=true;state.cacheMode=false;markMatchDataChanged();setDataStatus(`${state.matches.length.toLocaleString('ja-JP')}試合`);renderAll(deferSecondary);}
  async function loadMatches(){state.fullMatchesLoaded=false;const ok=await ensureFullMatches('全試合');if(ok&&state.session?.user&&state.ratingCacheAvailable)await persistRatingCache({silent:true});return ok;}
  ...
  const LAZY_SECTION_RENDERERS={
    'pref-share-trend':()=>renderPrefShareTrend(),
    'prefectures':()=>renderPrefCards(),
    'schools':()=>{renderSchoolSelect(false);renderSchoolProfile();},
    'rank-compare':()=>{renderRankCompareSelected();renderRankCompareChart();},
    'admin':()=>{renderAdminMatches();renderAliasSuggestions();syncSiteSettingsForm();}
  };
  function sectionNearViewport(el,margin=650){if(!el||el.classList.contains('hidden'))return false;const r=el.getBoundingClientRect();return r.bottom>=-margin&&r.top<=window.innerHeight+margin;}
  function renderLazySection(id,force=false){const fn=LAZY_SECTION_RENDERERS[id],el=document.getElementById(id);if(!fn||!el||el.classList.contains('hidden'))return;if(!force&&state.lazyRenderedSections.has(id))return;fn();state.lazyRenderedSections.add(id);}
  function renderVisibleLazySections(force=false){for(const id of Object.keys(LAZY_SECTION_RENDERERS)){const el=document.getElementById(id);if(sectionNearViewport(el))renderLazySection(id,force);}}
  function setupLazySectionRendering(){
    if(state.lazySectionObserver){renderVisibleLazySections(false);return;}
    if('IntersectionObserver' in window){
      state.lazySectionObserver=new IntersectionObserver(entries=>{for(const entry of entries){if(entry.isIntersecting){const id=entry.target.id;renderLazySection(id,false);}}},{rootMargin:'650px 0px'});
      for(const id of Object.keys(LAZY_SECTION_RENDERERS)){const el=document.getElementById(id);if(el)state.lazySectionObserver.observe(el);}
    }
    renderVisibleLazySections(false);
  }
  function renderSecondaryViews(){setupLazySectionRendering();renderVisibleLazySections(true);syncSiteSettingsForm();}
  function scheduleSecondaryViews(){
    ...
    const run=()=>{state.deferredRenderHandle=null;setupLazySectionRendering();};
    state.deferredRenderHandle='requestIdleCallback' in window?requestIdleCallback(run,{timeout:900}):setTimeout(run,250);
  }
  function renderAll(deferSecondary=false){hideSetupNotice();applyPublicVisibility();renderSummary();renderPrefFilter();renderRanking();syncSiteSettingsForm();if(deferSecondary)scheduleSecondaryViews();else renderSecondaryViews();}
  function hasOfficialGameInPastYear(school){
    if(school?.cacheOnly)return Boolean(school.activeLastYear);
  ...
  function renderSummary(){
    const matchCount=state.fullMatchesLoaded?state.matches.length:Number(state.cacheActualMeta?.match_count??state.cacheMeta?.match_count??0);
    const activeCount=state.fullMatchesLoaded?state.schools.filter(s=>!s.isJoint&&hasOfficialGameInPastYear(s)).length:Number(state.cacheMeta?.active_school_count||state.schools.filter(s=>!s.isJoint&&s.activeLastYear).length);
    const latest=state.fullMatchesLoaded?state.matches.map(m=>m.date).filter(Boolean).sort().at(-1):(state.cacheActualMeta?.latest_match_date||state.cacheMeta?.latest_match_date);
    els.matchCount.textContent=Number(matchCount||0).toLocaleString('ja-JP');els.schoolCount.textContent=Number(activeCount||0).toLocaleString('ja-JP');els.prefCount.textContent=state.prefs.length.toLocaleString('ja-JP');els.latestMatchDate.textContent=latest||'—';
  }
  function renderPrefFilter(){
    if(!els.rankingPrefOptions)return;
    const source=ratingVisibleSchools();
    const prefs=[...new Set(source.map(s=>canonicalPref(s.pref)).filter(Boolean))].sort(comparePrefectureOrder);
  ...
  function updateRankingPrefSummary(){
    ...
    const available=[...(els.rankingPrefOptions?.querySelectorAll('input[type="checkbox"]')||[])].map(input=>input.value);
    if(els.rankingPrefSelectAll)els.rankingPrefSelectAll.disabled=Boolean(available.length)&&available.every(pref=>state.selectedRankingPrefs.has(pref));
  }
  ...
  function historicalSchoolState(school,dateString){
    ...
    const games=(school.games||[]).filter(g=>String(g?.match?.date||'')<=dateString),last=h[idx];
  ...
  function renderRanking(){
    ...
    const latestDate=compareDatasetBounds().last||'';
    const date=state.rankingDate||latestDate||'';
    const source=(!state.fullMatchesLoaded&&(!date||date===latestDate))?ratingVisibleSchools():(date?rankingRowsAtDate(date):ratingVisibleSchools());
  ...
  function prefShareSnapshotDates(){
    const {first,last}=compareDatasetBounds();
    if(!first||!last)return[];
    const firstYear=Number(first.slice(0,4)),lastYear=Number(last.slice(0,4));
    if(!Number.isInteger(firstYear)||!Number.isInteger(lastYear))return[];
    const out=[];
    for(let year=firstYear;year<=lastYear;year++)out.push({year,date:year===lastYear?last:`${year}-12-31`,latest:year===lastYear});
    return out;
  }
  function publicRankingForShareAtDate(dateString){
    const key=`public|pref-share|${dateString}`;
    if(state.historicalRankingCache.has(key))return state.historicalRankingCache.get(key);
    const rows=[];
    for(const school of state.schools){
      const rating=ratingAtDate(school,dateString);
      if(rating===null||rating<PUBLIC_RATING_MIN)continue;
      rows.push({key:school.key,name:school.name,pref:canonicalPref(school.pref),rating,isJoint:school.isJoint});
    }
    rows.sort((a,b)=>b.rating-a.rating||a.name.localeCompare(b.name,'ja')||comparePrefectureOrder(a.pref,b.pref));
    state.historicalRankingCache.set(key,rows);
    return rows;
  }
  function readPrefShareTopN(){
    const n=Math.trunc(Number(els.prefShareTopN?.value||state.prefShareTopN||100));
    return Number.isFinite(n)?Math.min(1000,Math.max(1,n)):100;
  }
  function renderPrefShareTrend(){
    if(!els.prefShareHead||!els.prefShareBody)return;
    const n=readPrefShareTopN();
    state.prefShareTopN=n;
    if(els.prefShareTopN&&String(els.prefShareTopN.value)!==String(n))els.prefShareTopN.value=String(n);
    try{localStorage.setItem('kokoyakyu_pref_share_top_n',String(n));}catch(_){}
    if(els.prefShareTitle)els.prefShareTitle.textContent=`ベスト${n}に占める都道府県の割合推移`;
    if(!state.fullMatchesLoaded){
      const top=state.ranking.slice(0,n),denominator=top.length,counts=new Map();
      for(const school of top){const pref=canonicalPref(school.pref);if(pref)counts.set(pref,(counts.get(pref)||0)+1);}
      const year=String(state.cacheMeta?.latest_match_date||'').slice(0,4)||'最新';
      const sortDir=state.prefShareSortDir==='asc'?'asc':'desc';
      const prefs=[...PREFECTURE_ORDER];
      if(String(state.prefShareSortYear||'')===String(year))prefs.sort((a,b)=>{const av=counts.get(a)||0,bv=counts.get(b)||0,diff=sortDir==='asc'?av-bv:bv-av;return diff||comparePrefectureOrder(a,b);});
      const active=String(state.prefShareSortYear||'')===String(year),indicator=active?(sortDir==='desc'?'▼':'▲'):'↕';
      els.prefShareHead.innerHTML=`<tr><th class="pref-share-sticky">都道府県</th><th><button type="button" class="pref-share-sort-button${active?' active':''}" data-pref-share-sort-year="${escapeHtml(year)}" aria-label="${escapeHtml(year)}年の割合で並び替え">${escapeHtml(year)}（最新）<small>${denominator}校</small><span class="pref-share-sort-indicator" aria-hidden="true">${indicator}</span></button></th></tr>`;
      els.prefShareBody.innerHTML=prefs.map(pref=>{const count=counts.get(pref)||0,pct=denominator?count/denominator*100:0;return `<tr><th class="pref-share-sticky" scope="row">${escapeHtml(pref)}</th><td><span class="pref-share-count">${count}校</span><span class="pref-share-percent">${pct.toFixed(1)}%</span></td></tr>`;}).join('');
      if(els.prefShareStatus){els.prefShareStatus.innerHTML='最新年は高速表示キャッシュだけで表示しています。過去年度は必要な時だけ全試合を読み込みます。 <button id="prefShareLoadHistory" class="btn secondary small" type="button">過去推移を読み込む</button>';els.prefShareStatus.querySelector('#prefShareLoadHistory')?.addEventListener('click',async()=>{const b=els.prefShareStatus.querySelector('#prefShareLoadHistory');if(b)b.disabled=true;const ok=await ensureFullMatches('都道府県割合推移用の全試合');if(ok)renderPrefShareTrend();});}
      return;
    }
    const snapshots=prefShareSnapshotDates();
    if(!snapshots.length){
      els.prefShareHead.innerHTML='<tr><th>都道府県</th></tr>';
      els.prefShareBody.innerHTML='<tr><td class="empty">集計できる試合データがありません。</td></tr>';
      if(els.prefShareStatus)els.prefShareStatus.textContent='';
      return;
    }
    const columns=snapshots.map(s=>{
      const rows=publicRankingForShareAtDate(s.date),top=rows.slice(0,n),denominator=top.length,counts=new Map();
      for(const school of top){const pref=canonicalPref(school.pref);if(pref)counts.set(pref,(counts.get(pref)||0)+1);}
      return {...s,denominator,counts};
    });
    if(state.prefShareSortYear&&!columns.some(c=>String(c.year)===String(state.prefShareSortYear)))state.prefShareSortYear=null;
    const sortYear=state.prefShareSortYear?String(state.prefShareSortYear):null;
    const sortDir=state.prefShareSortDir==='asc'?'asc':'desc';
    els.prefShareHead.innerHTML=`<tr><th class="pref-share-sticky">都道府県</th>${columns.map(c=>{
      const active=sortYear===String(c.year),indicator=active?(sortDir==='desc'?'▼':'▲'):'↕';
      return `<th><button type="button" class="pref-share-sort-button${active?' active':''}" data-pref-share-sort-year="${c.year}" aria-label="${c.year}年の割合で並び替え">${c.year}${c.latest?'（最新）':''}<small>${c.denominator}校</small><span class="pref-share-sort-indicator" aria-hidden="true">${indicator}</span></button></th>`;
    }).join('')}</tr>`;
    let prefs=[...PREFECTURE_ORDER];
    if(sortYear){
      const col=columns.find(c=>String(c.year)===sortYear);
      if(col)prefs.sort((a,b)=>{
        const av=col.counts.get(a)||0,bv=col.counts.get(b)||0,diff=sortDir==='asc'?av-bv:bv-av;
        return diff||comparePrefectureOrder(a,b);
      });
    }
    els.prefShareBody.innerHTML=prefs.map(pref=>`<tr><th class="pref-share-sticky" scope="row">${escapeHtml(pref)}</th>${columns.map(c=>{const count=c.counts.get(pref)||0,pct=c.denominator?count/c.denominator*100:0;return `<td><span class="pref-share-count">${count}校</span><span class="pref-share-percent">${pct.toFixed(1)}%</span></td>`;}).join('')}</tr>`).join('');
    const latest=columns.at(-1),first=columns[0],sortNote=sortYear?` ${sortYear}年を${sortDir==='desc'?'降順':'昇順'}で表示中。`:'';
    if(els.prefShareStatus)els.prefShareStatus.textContent=`対象：公開ランキングの上位${n}校。${first.year}〜${latest.year}を比較（${latest.year}年は${latest.date}時点）。公開対象校が${n}校未満の年は、実際の掲載校数を母数にしています。${sortNote}`;
  }
  ...
  function renderSchoolSelect(forceFull=false){
    if(!els.schoolSelect)return;
    const previous=state.selectedSchoolKey||els.schoolSelect.value,source=ratingVisibleSchools();
    if(forceFull)state.schoolSelectPopulated=true;
    if(!state.schoolSelectPopulated){
      const current=resolveSchool(previous)||source[0]||null;
      const label=isAdmin()?'学校名検索を推奨（クリックで全校候補を読込）':'学校名検索を推奨（クリックで候補を読込）';
      els.schoolSelect.innerHTML=`<option value="">${label}</option>`+(current?`<option value="${escapeHtml(current.key)}">${escapeHtml(current.name)}（${escapeHtml(current.pref)}）</option>`:'');
      if(current){els.schoolSelect.value=current.key;state.selectedSchoolKey=current.key;}
      return;
    }
    els.schoolSelect.innerHTML=`<option value="">${isAdmin()?'全校から選択（管理者）':'Rating 1500以上から選択'}</option>`+source.map(s=>`<option value="${escapeHtml(s.key)}">${escapeHtml(s.name)}（${escapeHtml(s.pref)}）${isAdmin()&&s.rating<PUBLIC_RATING_MIN?' · '+formatRating(s.rating):''}</option>`).join('');
    if(source.some(s=>s.key===previous))els.schoolSelect.value=previous;
  }
  ...
  function openSchool(key,scroll=false){if(!featureVisible('school_details'))return;const school=resolveSchool(key);if(!school)return;state.selectedSchoolKey=school.key;renderSchoolSelect(false);if(state.schoolSelectPopulated&&ratingVisibleSchools().some(s=>s.key===school.key))els.schoolSelect.value=school.key;renderSchoolProfile();if(scroll){const target=document.querySelector('#schools');if(target)requestAnimationFrame(()=>target.scrollIntoView({behavior:'smooth',block:'start'}));}}
  function renderSchoolProfile(){
    const school=state.schoolMap.get(state.selectedSchoolKey)||state.schoolMap.get(els.schoolSelect.value)||ratingVisibleSchools()[0]||state.schools[0];
    if(!school){els.schoolName.textContent='試合データがありません';els.historyChart.innerHTML='';els.recentMatches.innerHTML='<div class="empty">試合データがありません。</div>';return;}
    state.selectedSchoolKey=school.key;els.schoolPref.textContent=school.pref;els.schoolName.textContent=school.name;
    const gameCount=state.fullMatchesLoaded?school.games.length:Number(school.wins||0)+Number(school.losses||0)+Number(school.draws||0);
    els.schoolRecord.textContent=`${gameCount}試合 ${school.wins}勝 ${school.losses}敗 ${school.draws}分${school.isJoint?' · 合同チーム':''}`;
    const ratingVisible=canViewSchoolRating(school);els.schoolRating.textContent=ratingVisible?formatRating(school.rating):'非公開';els.schoolRank.textContent=ratingVisible?`${isAdmin()?school.allRank:school.publicRank}位`:'—';if(els.schoolPrefRank)els.schoolPrefRank.textContent=ratingVisible&&school.prefRank?`${school.prefRank}位`:'—';els.schoolDelta.textContent=ratingVisible?formatDelta(school.lastDelta):'—';els.schoolDelta.className=ratingVisible?deltaClass(school.lastDelta):'';els.schoolForm.innerHTML=school.form.split('').map(r=>`<span class="match-result-${r}">${r}</span>`).join(' ')||'—';
    if(!state.fullMatchesLoaded){
      if(els.historyRangeStatus)els.historyRangeStatus.textContent='詳細履歴は必要なときだけ読み込みます。';
      els.historyChart.innerHTML='<text x="360" y="110" text-anchor="middle" class="chart-private">最新Ratingは高速表示中です</text>';
      renderMatchFilters([]);
      els.recentMatches.innerHTML='<div class="empty lazy-detail-box">試合履歴・Rating推移は全試合データを読み込むと表示できます。<br><button id="schoolLoadFullHistory" class="btn secondary small" type="button">この学校の履歴を読み込む</button></div>';
      els.recentMatches.querySelector('#schoolLoadFullHistory')?.addEventListener('click',async e=>{e.currentTarget.disabled=true;const key=school.key;const ok=await ensureFullMatches('学校詳細用の全試合');if(ok){state.selectedSchoolKey=key;renderSchoolProfile();}});
      return;
    }
    renderHistoryChart(school.history,ratingVisible);renderMatchFilters(school.games);renderRecentMatches(school.games,ratingVisible);
  }
  ...
  function renderRecentMatches(games,publicRating=true){
    ...
    els.recentMatches.innerHTML=list.map(game=>{const m=game.match,source=m.source_url?` · <a href="${escapeHtml(m.source_url)}" target="_blank" rel="noopener noreferrer">出典</a>`:'',kText=showK?` · K=${inferK(m)}`:'',regressionText=publicRating&&Number(game.postNewTeamTransitions)>0?`<div class="match-meta">試合終了後に平均回帰 ${game.postNewTeamTransitions}回：${formatRating(game.matchAfter)} → ${formatRating(game.regressionTo)}</div>`:'';return `<div class="match-row"><div class="match-meta">${escapeHtml(m.date)} · ${escapeHtml(m.tournament)} ${escapeHtml(m.stage||'')}${source}${kText}</div><strong class="match-result-${game.result}">${escapeHtml(game.result)}　<button class="school-link js-open-opponent" type="button">${escapeHtml(game.opponent)}</button>　${game.scored} - ${game.allowed}</strong>${publicRating?`<div class="match-meta">試合による変動：${formatRating(game.before)} → ${formatRating(game.after)}（${formatDelta(game.delta)}）</div>${regressionText}`:''}<div class="match-actions">${state.session?.user?`<button class="btn secondary small js-edit-record" data-id="${escapeHtml(m.id)}" type="button">この戦績を編集</button>`:''}</div></div>`;}).join('');
  ...
  function latestDatasetDate(){if(!state.fullMatchesLoaded&&state.cacheMeta?.latest_match_date)return parseLocalDate(state.cacheMeta.latest_match_date);const dates=state.matches.map(m=>m.date).filter(Boolean).sort();return dates.length?parseLocalDate(dates.at(-1)):new Date();}
  function historyWindow(history){
    const actual=[...history].filter(h=>h.date).sort((a,b)=>String(a.date).localeCompare(String(b.date)));
    const matchCount=points=>points.filter(p=>p.eventType!=='regression').length;
    ...
    if(state.historyYears==='all'){
      ...
      return {points,actualCount:matchCount(actual),start,end,label:'全期間'};
    }
    if(state.historyYears==='newteam'){
      const autumnStarts=actual.filter(p=>p.eventType!=='regression'&&p.isAutumnFirstGame);
      ...
      return {points,actualCount:matchCount(inRange),start,end,label:'新体制以降'};
    }
    ...
    points.push(...inRange);
    if(points.length){const last=points.at(-1);if(parseLocalDate(last.date)<end)points.push({...last,date:toDateString(end),synthetic:true,opponent:null});}
    return {points,actualCount:matchCount(inRange),start,end,label:`過去${state.historyYears}年`};
  }
  function regressionTooltip(p){
    const timing=p.regressionTiming==='after_summer_final'?'夏の最終戦直後':'未反映分を次回出場前に適用';
    return `${p.date} 平均回帰 ${Number(p.newTeamTransitions)||1}回（${timing}）：${formatRating(p.regressionFrom)} → ${formatRating(p.regressionTo)}`;
  }
  function renderHistoryChart(history,ratingVisible=true){
    ...
    const dots=h.map((p,i)=>{
      ...
      if(p.eventType==='regression'){
        const r=5.2,title=escapeHtml(regressionTooltip(p));
        return `<polygon class="chart-regression-dot" points="${cx},${cy-r} ${cx+r},${cy} ${cx},${cy+r} ${cx-r},${cy}"><title>${title}</title></polygon>`;
      }
      return `<circle class="chart-dot" cx="${cx}" cy="${cy}" r="2.8"><title>${p.date} 試合終了時 レーティング ${formatRating(p.rating)}${opponent}</title></circle>`;
    }).join('');
  ...
  function compareDatasetBounds(){
    if(!state.fullMatchesLoaded&&state.cacheMeta)return {first:state.cacheMeta.first_match_date||null,last:state.cacheMeta.latest_match_date||null};
    const dates=state.matches.map(m=>m.date).filter(Boolean).sort();
  ...
  function renderRankCompareChart(){
    ...
    if(!state.fullMatchesLoaded){
      els.rankCompareChart.innerHTML='<text x="450" y="170" text-anchor="middle" class="chart-private">順位推移は必要なときだけ全試合を読み込みます</text>';els.rankCompareLegend.innerHTML='';
      if(els.rankCompareStatus){els.rankCompareStatus.innerHTML='最新Ratingは高速表示中です。 <button id="rankCompareLoadHistory" class="btn secondary small" type="button">順位推移を読み込む</button>';els.rankCompareStatus.querySelector('#rankCompareLoadHistory')?.addEventListener('click',async e=>{e.currentTarget.disabled=true;const ok=await ensureFullMatches('順位推移用の全試合');if(ok){renderRankCompareSelected();renderRankCompareChart();}});}
      return;
    }
  ...
  function renderMergeProposalSearch(side){
    ...
    refs.results.innerHTML=hits.length?hits.map((s,i)=>`<button class="school-search-item js-merge-proposal-school" type="button" data-index="${i}"><strong>${escapeHtml(s.name)}</strong><span>${escapeHtml(s.pref)} · ${(s.games?.length||(Number(s.wins||0)+Number(s.losses||0)+Number(s.draws||0)))}試合</span></button>`).join(''):'<div class="school-search-empty">該当する学校がありません。</div>';
  ...
  function renderResultProposalSearch(){
    ...
    if(!state.fullMatchesLoaded){els.resultProposalMatchResults.innerHTML='<div class="school-search-empty">試合検索には詳細データが必要です。<br><button id="resultProposalLoadMatches" class="btn secondary small" type="button">試合データを読み込む</button></div>';els.resultProposalMatchResults.classList.remove('hidden');els.resultProposalMatchResults.querySelector('#resultProposalLoadMatches')?.addEventListener('click',async e=>{e.currentTarget.disabled=true;const ok=await ensureFullMatches('修正提案用の全試合');if(ok)renderResultProposalSearch();});return;}
  ...
  async function handleSiteSettingsSave(e){
    ...
    if(!state.fullMatchesLoaded){const ok=await ensureFullMatches('Rating再計算用の全試合');if(!ok)return;}
    const fields={
    ...
    renderStaticConfig();renderAll();await persistRatingCache({silent:true});setMessage(els.siteSettingsMessage,'K係数と新チーム継承率を保存しました。Ratingを再計算し、高速表示キャッシュも更新しました。','success');
  }
  ...
  async function handleMatchSubmit(e){
    ...
    if(!state.fullMatchesLoaded){setMessage(els.matchFormMessage,'試合データを読み込み中…');const ok=await ensureFullMatches('試合登録用の全試合');if(!ok)return setMessage(els.matchFormMessage,'試合データを読み込めませんでした。','error');}
  ...
  async function deleteMatch(id){const m=state.matches.find(x=>String(x.id)===String(id));if(!m||!confirm(`${m.date} ${m.team_a} ${m.score_a}-${m.score_b} ${m.team_b}\nこの試合を削除しますか？`))return;const {error}=await state.client.from('matches').delete().eq('id',id);if(error)return alert(error.message);state.matches=state.matches.filter(x=>String(x.id)!==String(id));rebuildFromLocalMatches(true);await persistRatingCache({silent:true});await loadEditHistory();}
  ...
  function renderAdminMatches(){
    ...
    if(!state.fullMatchesLoaded){
      els.adminMatchesBody.innerHTML='<tr><td colspan="4" class="empty">試合一覧は必要な時だけ読み込みます。<br><button id="adminLoadMatches" class="btn secondary small" type="button">試合データを読み込む</button></td></tr>';
      if(els.adminMatchSearchStatus)els.adminMatchSearchStatus.textContent='最新Ratingは高速表示中です。試合一覧を操作する時だけ全試合を読み込みます。';
      els.adminMatchesBody.querySelector('#adminLoadMatches')?.addEventListener('click',async e=>{e.currentTarget.disabled=true;const ok=await ensureFullMatches('管理画面の試合一覧用の全試合');if(ok)renderAdminMatches();});
      return;
    }
  ...
  async function loadSchoolAliases(){
    ...
    if(state.ready&&state.fullMatchesLoaded)rebuildFromLocalMatches(true);
  }
  ...
  function schoolAliasGroups(){
    return state.schoolMasters.map(master=>({key:schoolAliasGroupKey(master),masterId:String(master.id),master,prefecture:aliasPrefKey(master.prefecture),canonical_name:String(master.display_name||master.official_name||'').trim(),rows:state.schoolAliases.filter(a=>String(a.school_id||'')===String(master.id)),aliases:namesForSchoolMaster(master)})).sort((a,b)=>comparePrefectureOrder(a.prefecture,b.prefecture)||a.canonical_name.localeCompare(b.canonical_name,'ja'));
  }
  ...
  async function addSchoolAlias(){
    ...
    resetSchoolAliasEditor(true);await loadSchoolAliases();rebuildFromLocalMatches(true);await persistRatingCache({silent:true});setMessage(els.aliasMessage,oldGroup?'学校名辞書を更新しました。登録済み名称はすべて保持・反映されています。':'学校名辞書に登録しました。','success');
  }
  ...
  function debounce(fn,delay=160){let timer=null;return (...args)=>{clearTimeout(timer);timer=setTimeout(()=>fn(...args),delay);};}
  const renderRankingDebounced=debounce(()=>{state.rankingPage=1;renderRanking();},140);
  const renderPrefShareDebounced=debounce(()=>renderPrefShareTrend(),180);
  const renderSchoolSearchDebounced=debounce(()=>renderSchoolSearch(),120);
  const renderRecordSearchDebounced=debounce(()=>renderRecordSchoolSearch(),120);

  function bindEvents(){els.siteSettingsForm?.addEventListener('submit',handleSiteSettingsSave);document.querySelectorAll('.admin-public-toggle [data-setting]').forEach(input=>input.addEventListener('change',()=>handlePublicSettingChange(input)));els.searchInput.addEventListener('input',renderRankingDebounced);els.rankingPrefOptions?.addEventListener('change',e=>{const input=e.target.closest('input[type=\"checkbox\"]');if(!input)return;if(input.checked)state.selectedRankingPrefs.add(input.value);else state.selectedRankingPrefs.delete(input.value);updateRankingPrefSummary();state.rankingPage=1;renderRanking();});els.rankingPrefSelectAll?.addEventListener('click',()=>{const prefs=[...(els.rankingPrefOptions?.querySelectorAll('input[type=\"checkbox\"]')||[])].map(input=>input.value);state.selectedRankingPrefs=new Set(prefs);renderPrefFilter();state.rankingPage=1;renderRanking();});els.rankingPrefClear?.addEventListener('click',()=>{state.selectedRankingPrefs.clear();renderPrefFilter();state.rankingPage=1;renderRanking();});els.prefShareTopN?.addEventListener('input',()=>{if(String(els.prefShareTopN.value||'').trim())renderPrefShareDebounced();});els.prefShareTopN?.addEventListener('change',()=>renderPrefShareTrend());els.prefShareHead?.addEventListener('click',e=>{const b=e.target.closest('[data-pref-share-sort-year]');if(!b)return;const year=String(b.dataset.prefShareSortYear||'');if(state.prefShareSortYear===year)state.prefShareSortDir=state.prefShareSortDir==='desc'?'asc':'desc';else{state.prefShareSortYear=year;state.prefShareSortDir='desc';}renderPrefShareTrend();});[els.ratingMinFilter,els.ratingMaxFilter].forEach(x=>x?.addEventListener('input',renderRankingDebounced));els.rankingDate?.addEventListener('change',async()=>{state.rankingDate=els.rankingDate.value;state.rankingDateIsLatest=state.rankingDate===compareDatasetBounds().last;state.rankingPage=1;if(!state.rankingDateIsLatest&&!state.fullMatchesLoaded){const ok=await ensureFullMatches('過去ランキング用の全試合');if(!ok)return;}renderRanking();});els.rankingDateLatest?.addEventListener('click',()=>{state.rankingDateIsLatest=true;syncRankingDateInput(true);state.rankingPage=1;renderRanking();});els.rankingPageSize?.addEventListener('change',()=>{state.rankingPageSize=Number(els.rankingPageSize.value)||20;state.rankingPage=1;renderRanking();});els.rankingPrev?.addEventListener('click',()=>{if(state.rankingPage>1){state.rankingPage--;renderRanking();document.getElementById('ranking')?.scrollIntoView({behavior:'smooth',block:'start'});}});els.rankingNext?.addEventListener('click',()=>{state.rankingPage++;renderRanking();document.getElementById('ranking')?.scrollIntoView({behavior:'smooth',block:'start'});});els.rankingPageNumbers?.addEventListener('click',e=>{const b=e.target.closest('[data-page]');if(b)goToRankingPage(b.dataset.page);});els.prefSort?.addEventListener('change',()=>{state.prefExpanded=false;renderPrefCards();});els.prefShowMoreButton?.addEventListener('click',()=>{state.prefExpanded=!state.prefExpanded;renderPrefCards();});els.schoolSearch?.addEventListener('input',renderSchoolSearchDebounced);els.schoolSearch?.addEventListener('keydown',e=>{if(e.key==='Enter'&&state.schoolSearchHits.length){e.preventDefault();selectSchoolSearchHit(0);}});els.recordSchoolSearch?.addEventListener('input',renderRecordSearchDebounced);els.recordSchoolSearch?.addEventListener('keydown',e=>{if(e.key==='Enter'&&state.recordSearchHits.length){e.preventDefault();selectRecordSearchHit(0);}});els.schoolSelect.addEventListener('focus',()=>{if(!state.schoolSelectPopulated)renderSchoolSelect(true);},{once:true});els.schoolSelect.addEventListener('change',()=>{state.selectedSchoolKey=els.schoolSelect.value;renderSchoolProfile();});els.historyRange?.addEventListener('click',async e=>{const b=e.target.closest('[data-years]');if(!b)return;state.historyYears=b.dataset.years;els.historyRange.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x===b));if(!state.fullMatchesLoaded){const key=state.selectedSchoolKey;const ok=await ensureFullMatches('学校履歴用の全試合');if(!ok)return;if(key)state.selectedSchoolKey=key;}renderSchoolProfile();});els.matchYearFilter?.addEventListener('change',async()=>{if(!state.fullMatchesLoaded){const key=state.selectedSchoolKey;const ok=await ensureFullMatches('学校戦績用の全試合');if(!ok)return;if(key)state.selectedSchoolKey=key;}renderSchoolProfile();});els.rankCompareSearch?.addEventListener('input',renderRankCompareSearch);els.rankCompareSearch?.addEventListener('keydown',e=>{if(e.key==='Enter'&&state.rankCompareSearchHits.length){e.preventDefault();addRankCompareSchool(0);}});els.rankCompareStartDate?.addEventListener('change',()=>{state.compareStartDate=els.rankCompareStartDate.value;renderRankCompareChart();});els.rankCompareEndDate?.addEventListener('change',()=>{state.compareEndDate=els.rankCompareEndDate.value;renderRankCompareChart();});els.rankCompareAllRange?.addEventListener('click',()=>{syncRankCompareDateInputs(true);renderRankCompareChart();});[els.ratingA,els.ratingB,els.kValue].forEach(i=>i.addEventListener('input',renderSimulator));els.kValue?.addEventListener('input',()=>{els.kValue.dataset.userEdited='1';});els.loginForm.addEventListener('submit',handleLogin);els.showResetButton.addEventListener('click',showResetRequest);els.resetRequestForm.addEventListener('submit',handleResetRequest);els.backToLoginButton.addEventListener('click',backToLogin);els.passwordSetupForm.addEventListener('submit',handlePasswordSetup);els.logoutButton.addEventListener('click',handleLogout);els.ratingCacheRebuildButton?.addEventListener('click',rebuildRatingCache);els.matchForm.addEventListener('submit',handleMatchSubmit);els.cancelEditButton.addEventListener('click',resetMatchForm);els.reloadButton.addEventListener('click',loadMatches);[els.adminMatchKeyword,els.adminMatchTournament,els.adminMatchSchool].forEach(x=>x?.addEventListener('input',renderAdminMatches));els.adminMatchDate?.addEventListener('change',renderAdminMatches);els.adminMatchClear?.addEventListener('click',clearAdminMatchSearch);els.duplicateScanButton?.addEventListener('click',async()=>{if(!state.fullMatchesLoaded){const ok=await ensureFullMatches('重複検知用の全試合');if(!ok)return;}toggleDuplicateView();});els.duplicateSelectAll?.addEventListener('change',()=>setAllDuplicateChecks(els.duplicateSelectAll.checked));els.duplicateMergeList?.addEventListener('change',e=>{if(e.target.closest('.js-merge-duplicate'))syncDuplicateSelectionState();});els.duplicateMergeButton?.addEventListener('click',mergeCheckedDuplicates);els.normalizeTournamentButton?.addEventListener('click',async()=>{if(!state.fullMatchesLoaded){const ok=await ensureFullMatches('大会名修正用の全試合');if(!ok)return;}normalizeTournamentNames();});els.aliasAddButton?.addEventListener('click',addSchoolAlias);els.aliasAddNameButton?.addEventListener('click',()=>{const names=[...(els.aliasNamesContainer?.querySelectorAll('.alias-name-input')||[])].map(x=>x.value);names.push('');renderSchoolAliasNameInputs(names);const inputs=els.aliasNamesContainer?.querySelectorAll('.alias-name-input');inputs?.[inputs.length-1]?.focus();});els.aliasNamesContainer?.addEventListener('click',e=>{const b=e.target.closest('.alias-name-remove');if(!b)return;const row=b.closest('.alias-name-row');row?.remove();const names=[...(els.aliasNamesContainer?.querySelectorAll('.alias-name-input')||[])].map(x=>x.value);renderSchoolAliasNameInputs(names.length?names:['']);});els.aliasCancelEditButton?.addEventListener('click',()=>resetSchoolAliasEditor(true));els.aliasRefreshButton?.addEventListener('click',async()=>{if(!state.fullMatchesLoaded){const ok=await ensureFullMatches('学校名候補検出用の全試合');if(!ok)return;}renderAliasSuggestions();if(els.aliasMessage)setMessage(els.aliasMessage,'登録済み試合を再検出しました。','success');});els.aliasListPrefFilter?.addEventListener('change',renderAliasList);els.tournamentAliasSaveButton?.addEventListener('click',saveTournamentAlias);els.tournamentAliasCancelEditButton?.addEventListener('click',()=>resetTournamentAliasEditor(true));els.tournamentAliasRefreshButton?.addEventListener('click',loadTournamentAliases);els.tournamentAliasYearFilter?.addEventListener('change',renderTournamentAliasList);els.schoolMergeProposalForm?.addEventListener('submit',submitSchoolMergeProposal);els.schoolMergeProposalType?.addEventListener('change',()=>{clearMergeProposalForm(false);updateProposalMode();});els.mergeProposalSchoolA?.addEventListener('input',()=>renderMergeProposalSearch('A'));els.mergeProposalSchoolB?.addEventListener('input',()=>renderMergeProposalSearch('B'));els.mergeProposalSchoolA?.addEventListener('keydown',e=>{if(e.key==='Enter'&&state.mergeProposalAHits.length){e.preventDefault();selectMergeProposalSchool('A',0);}});els.mergeProposalSchoolB?.addEventListener('keydown',e=>{if(e.key==='Enter'&&state.mergeProposalBHits.length){e.preventDefault();selectMergeProposalSchool('B',0);}});els.resultProposalMatchSearch?.addEventListener('input',renderResultProposalSearch);els.resultProposalMatchSearch?.addEventListener('keydown',e=>{if(e.key==='Enter'&&state.resultProposalHits.length){e.preventDefault();selectResultProposalMatch(0);}});[els.resultProposalScoreA,els.resultProposalScoreB].forEach(x=>x?.addEventListener('input',updateResultProposalSelection));els.schoolMergeProposalClear?.addEventListener('click',()=>clearMergeProposalForm(false));els.schoolReplacePreviewButton?.addEventListener('click',renderSchoolNameReplacementPreview);els.schoolReplaceApplyButton?.addEventListener('click',applySchoolNameReplacement);[els.schoolReplaceFrom,els.schoolReplaceTo].forEach(x=>x?.addEventListener('input',resetSchoolNameReplacementPreview));els.schoolReplaceMode?.addEventListener('change',resetSchoolNameReplacementPreview);els.reloadProposalsButton?.addEventListener('click',loadProposals);els.reloadHistoryButton?.addEventListener('click',loadEditHistory);document.addEventListener('click',e=>{if(els.schoolSearchResults&&!e.target.closest('.school-search-wrap'))els.schoolSearchResults.classList.add('hidden');if(els.recordSchoolSearchResults&&!e.target.closest('.record-school-search-wrap'))els.recordSchoolSearchResults.classList.add('hidden');if(els.rankCompareSearchResults&&!e.target.closest('.rank-compare-search-wrap'))els.rankCompareSearchResults.classList.add('hidden');if(!e.target.closest('.proposal-school-search-wrap')&&!e.target.closest('.proposal-match-search-wrap')){els.mergeProposalSchoolAResults?.classList.add('hidden');els.mergeProposalSchoolBResults?.classList.add('hidden');els.resultProposalMatchResults?.classList.add('hidden');}});}

  function renderStaticConfig(){const showK=featureVisible('k_values'),vals=configuredKValues(),lo=Math.min(...vals),hi=Math.max(...vals),retentionPct=(newTeamRetentionRate()*100).toFixed(0);els.heroInitial.textContent=ratingCfg.initial;els.heroDivisor.textContent=ratingCfg.divisor;els.heroK.textContent=showK?`大会別 ${lo}〜${hi}`:'非公開';if(els.heroRetention)els.heroRetention.textContent=`${retentionPct}%`;els.heroFormula.textContent="R' = R + K × (W − We)";if(!els.kValue.dataset.userEdited)els.kValue.value=showK?summerQualifierK():ratingCfg.defaultK;if(els.matchK)els.matchK.placeholder='大会タグから自動決定（空欄でOK）';if(els.matchKValues)els.matchKValues.innerHTML=[...new Set(vals)].sort((a,b)=>a-b).map(v=>`<option value="${v}"></option>`).join('');if(els.methodKText)els.methodKText.textContent=showK?`K値：秋季予選${autumnQualifierK()} / 秋季地区${autumnRegionalK()} / 神宮${meijiJinguK()} / 春甲子園1回戦〜準々決勝${springKoshienEarlyK()} / 春甲子園準決勝・決勝${springKoshienFinalK()} / 春季予選${springQualifierK()} / 春季地区${springRegionalK()} / 夏予選${summerQualifierK()} / 夏甲子園1回戦〜準々決勝${summerMainEarlyK()} / 夏甲子園準決勝・決勝${summerMainFinalK()} / 国スポ${kokuspoK()}。大会名ではなく各試合の大会タグで決定し、手動Kがある場合だけ手動値を優先します。`:'K値は各試合の大会タグに基づいて自動決定します。';if(els.methodRetentionText)els.methodRetentionText.textContent=`3年生引退後の新チームでは、1500からの差の${retentionPct}%を引き継ぎます。原則として各校のその年の夏大会最後の試合結果をまず通常どおり確定し、その直後に別イベントとして平均回帰を適用します。夏大会に出場しなかった年度は、次に公式戦へ出場する直前に未反映の世代交代回数をまとめて適用します。`;syncSiteSettingsForm();}
  async function init(){bindEvents();updateProposalMode();renderStaticConfig();renderSimulator();resetMatchForm();if(!isConfigReady()){setDataStatus('要設定');showSetupNotice('<strong>Supabaseの接続情報が未設定です。</strong> config.js を設定してください。');els.loginPanel.classList.add('hidden');els.adminUnavailable.textContent='config.jsを設定すると利用できます。';els.adminUnavailable.classList.remove('hidden');if(!globalRatingReadyResolved&&resolveGlobalRatingReady){globalRatingReadyResolved=true;resolveGlobalRatingReady(null);}return;}state.client=window.supabase.createClient(cfg.supabaseUrl,cfg.supabasePublishableKey);els.adminUnavailable.classList.add('hidden');await Promise.all([loadSiteSettings(),loadTournamentKSettings()]);await restoreSession();const cached=await loadRatingCache();if(!cached){setDataStatus('キャッシュ未作成');showSetupNotice('<strong>高速表示キャッシュがありません。</strong> 管理者でログインし「全試合から再計算して更新」を1回実行してください。通常閲覧では全試合を自動読み込みしません。');if(!globalRatingReadyResolved&&resolveGlobalRatingReady){globalRatingReadyResolved=true;resolveGlobalRatingReady(null);}}}
