(() => {
  ...
  const state = {
    ...
    ranking: [], prefs: [], proposals: [], editHistory: [], schoolAliases: [], schoolAliasMap: new Map(), aliasSuggestions: [], aliasesAvailable: true, ready: false, historyYears: '1', compareStartDate: '', compareEndDate: '', rankingDate: '', rankingDateIsLatest: true, selectedSchoolKey: null, schoolSearchHits: [], recordSearchHits: [], compareSchoolKeys: [], rankCompareSearchHits: [], showDuplicatesOnly: false, duplicateGroups: [], prefExpanded: false, rankingPage: 1, rankingPageSize: 20, rankSnapshotCache: new Map(), historicalRankingCache: new Map(), deferredRenderHandle: null, duplicateCacheDirty: true, matchSortCache: null,
  ...
  function inferTournamentTypeLegacy(match){
    ...
    if(/春の甲子園|選抜|センバツ/.test(t)) return 'spring_koshien';
    if(/(?:^|年)夏甲子園|夏の甲子園/.test(t) && !/予選|地方大会|支部大会|県大会|府大会|都大会/.test(t)) return 'summer_main';
    const summerNational=t.replace(/^第\d+回/,'');
    if(/^全国(?:高等学校|高校)野球選手権大会$/.test(summerNational)) return 'summer_main';
  ...
  function springKoshienK(){return kForType('spring_koshien');}
  ...
  function summerMainK(){return kForType('summer_main');}
  ...
  function renderRecentMatches(games,publicRating=true){
    ...
    const showK=featureVisible('k_values'),showProposal=featureVisible('correction_proposals');
    els.recentMatches.innerHTML=list.map(game=>{const m=game.match,source=m.source_url?` · <a href="${escapeHtml(m.source_url)}" target="_blank" rel="noopener noreferrer">出典</a>`:'',kText=showK?` · K=${inferK(m)}`:'';return `<div class="match-row"><div class="match-meta">${escapeHtml(m.date)} · ${escapeHtml(m.tournament)} ${escapeHtml(m.stage||'')}${source}${kText}</div><strong class="match-result-${game.result}">${escapeHtml(game.result)}　<button class="school-link js-open-opponent" type="button">${escapeHtml(game.opponent)}</button>　${game.scored} - ${game.allowed}</strong>${publicRating?`<div class="match-meta">${formatRating(game.before)} → ${formatRating(game.after)}（${formatDelta(game.delta)}）</div>`:''}<div class="match-actions">${state.session?.user?`<button class="btn secondary small js-edit-record" data-id="${escapeHtml(m.id)}" type="button">この戦績を編集</button>`:''}${showProposal?`<button class="btn secondary small js-propose" data-id="${escapeHtml(m.id)}" type="button">修正を提案</button>`:''}</div><div class="proposal-slot" data-proposal-slot="${escapeHtml(m.id)}"></div></div>`;}).join('');
  ...
  function syncSiteSettingsForm(){
    ...
    const pairs=[
      ...
      [els.siteSpringQualifierK,springQualifierK()],[els.siteSpringRegionalK,springRegionalK()],[els.siteSummerQualifierK,summerQualifierK()],[els.siteSummerMainK,summerMainK()],[els.siteKokuspoK,kokuspoK()]
    ];
  ...
  async function handleSiteSettingsSave(e){
    ...
    const fields={
      autumn_qualifier:els.siteAutumnQualifierK,autumn_regional:els.siteAutumnRegionalK,meiji_jingu:els.siteMeijiJinguK,spring_koshien:els.siteSpringKoshienK,
      spring_qualifier:els.siteSpringQualifierK,spring_regional:els.siteSpringRegionalK,summer_qualifier:els.siteSummerQualifierK,summer_main:els.siteSummerMainK,kokuspo:els.siteKokuspoK
    };
    const values={};
  ...
  function rebuildSchoolAliasMap(){state.schoolAliasMap=new Map();for(const a of state.schoolAliases){const key=schoolAliasLookupKey(a.prefecture,a.alias_name);if(key&&!key.endsWith('||')&&aliasNameKey(a.canonical_name)!==aliasNameKey(a.alias_name))state.schoolAliasMap.set(key,a);}}
  function findKnownAlias(name,pref){
    const key=schoolAliasLookupKey(pref,name);if(!key||key.endsWith('||'))return null;
    return state.schoolAliasMap.get(key)||null;
  }
  async function loadSchoolAliases(){
    if(!state.client||!state.session?.user||!els.aliasList)return;
    const {data,error}=await state.client.from('school_aliases').select('id,prefecture,alias_name,canonical_name,created_at').order('prefecture',{ascending:true}).order('alias_name',{ascending:true});
    if(error){state.schoolAliases=[];state.schoolAliasMap=new Map();state.aliasSuggestions=[];state.aliasesAvailable=false;renderAliasList();renderAliasSuggestions();if(els.aliasMessage)els.aliasMessage.textContent='別名辞書テーブルがありません。upgrade-school-aliases.sql を一度実行してください。';return;}
    state.schoolAliases=data||[];rebuildSchoolAliasMap();state.aliasesAvailable=true;if(els.aliasMessage)els.aliasMessage.textContent='';renderAliasList();renderAliasSuggestions();
  }
  function renderAliasList(){
    ...
    els.aliasList.innerHTML=state.schoolAliases.map(a=>`<div class="school-alias-item"><div class="school-alias-item-main"><strong>${escapeHtml(a.prefecture)}：${escapeHtml(a.alias_name)} <span class="school-alias-arrow">→</span> ${escapeHtml(a.canonical_name)}</strong><small>この都道府県内だけで照合</small></div><button class="btn secondary small js-alias-delete" type="button" data-id="${escapeHtml(a.id)}">削除</button></div>`).join('');
  ...
  async function addSchoolAlias(){
    ...
    const {error}=await state.client.from('school_aliases').upsert({prefecture,alias_name,canonical_name,updated_at:new Date().toISOString()},{onConflict:'prefecture,alias_name'});
    if(error)return setMessage(els.aliasMessage,`登録できませんでした: ${error.message}`,'error');
    if(els.aliasPref)els.aliasPref.value=prefecture;if(els.aliasName)els.aliasName.value='';if(els.aliasCanonical)els.aliasCanonical.value='';
    await loadSchoolAliases();setMessage(els.aliasMessage,'別名を登録しました。登録済み試合を再検出しました。','success');
  }
  async function deleteSchoolAlias(id){
    ...
    const {error}=await state.client.from('school_aliases').delete().eq('id',id);if(error)return alert(error.message);await loadSchoolAliases();
  }
  ...
  function applyKnownAliasesToPayload(payload){
    ...
    for(const side of ['a','b']){
      const teamField=`team_${side}`,prefField=`pref_${side}`,alias=findKnownAlias(p[teamField],p[prefField]);
      if(!alias)continue;changes.push(`${alias.prefecture}：${p[teamField]} → ${alias.canonical_name}`);p[teamField]=alias.canonical_name;
    }
